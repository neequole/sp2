...
if(isset($_SESSION['loggedin']) && $_SESSION['loggedin']=="YES"){
			...
			echo "<tr><th>Event</th><th>Date</th><th>Time</th><th>Venue</th><th>Status</th><th>Action</th></tr>";
			if(isset($count) && $count > 0){
					while($row3 = mysql_fetch_array($qry)){
						//print_r($row3);
						if($row3["status"] == "pending") echo "<tr id='".$row3["book_id"]."'><td>".$row3["title"]."</td><td>".$row3["e_date"]."</td><td>".$row3["e_stime"]." - ".$row3["e_etime"]."</td><td>".$row3["venue_name"]."</td><td>".$row3["status"]."</td><td><input type='button' id='cancel_booking' name='".$row3["book_id"]."'value='cancel'/></td></tr>";
						else echo "<tr id='".$row3["book_id"]."'><td>".$row3["title"]."</td><td>".$row3["e_date"]."</td><td>".$row3["e_stime"]." - ".$row3["e_etime"]."</td><td>".$row3["venue_name"]."</td><td>".$row3["status"]."</td><td></td></tr>";
					}
			}
