...
if(isset($_SESSION['loggedin']) && $_SESSION['loggedin']=="YES"){
			...
			echo "<table id='listBooking' class='table_center' cellpadding='10'>";
			...
			if(isset($count) && $count > 0){
					while($row3 = mysql_fetch_array($qry)){
						//print_r($row3);
						if($row3["status"] == "pending") echo "<tr id='".$row3["book_id"]."'><td>".$row3["title"]."</td><td>".$row3["e_date"]."</td><td>".$row3["e_stime"]." - ".$row3["e_etime"]."</td><td>".$row3["venue_name"]."</td><td>".$row3['e_tclass']."</td><td>".$row3["status"]."</td><td><input type='button' id='cancel_booking' class='cancel_button' name='".$row3["book_id"]."'value='cancel'/></td></tr>";
